...
upgrade: piptools upgrade.pins upgrade.envs upgrade.piptools ## re-compile requirements .txt files from .in file, upgrading packages

upgrade.piptools:
	pip-compile requirements/pip-tools.in --output-file=requirements/pip-tools.txt

upgrade.pins:
	# We must first make pins.txt blank, otherwise no package pins will be
	# upgraded, because they will be stuck to the old pins in pins.txt.
	echo > requirements/pins.txt
	pip-compile requirements/pins.in --upgrade --output-file=requirements/pins.txt
	
upgrade.envs: \
	upgrade.env.test \
	upgrade.env.docs \
	upgrade.env.local \
	upgrade.env.devstack \
	upgrade.env.production \

upgrade.env.%:
	pip-compile requirements/$*.env.in --output-file=requirements/$*.env.txt
...
production-requirements: requirements.production ## install requirements for production

prod-requirements: requirements.production ## synonymous to 'production-requirements'

devstack-requirements: requirements.devstack ## install requirements for devstack development
