...
upgrade: piptools upgrade-all  ## re-compile requirements .txt files from .in file, upgrading packages

# Note: order matters here. Dependees before dependents.
upgrade-all: \
	upgrade.pip-tools \
	upgrade.base \
	upgrade.test \
	...
	upgrade.production \
	upgrade.optional \
	upgrade-monitoring-requirements

upgrade-monitoring-requirements:
	pip-compile --upgrade --output-file=requirements/monitoring/requirements.txt \
		requirements/monitoring/requirements.in
