...
from ..data import DISCOVERY_PROGRAM_API_TPL, DiscoveryCourseRun, DiscoveryProgram
...
class GetDiscoveryProgramTestCase(TestCase):
    ...
    discovery_url = urljoin(
        settings.DISCOVERY_BASE_URL, DISCOVERY_PROGRAM_API_TPL.format(program_uuid)
    )
    course_run_1 = {
        "key": "0001",
        "uuid": "0000-0001",
    ...
    def assert_discovery_programs_equal(self, this_program, that_program):
        ...
        self.assertEqual(
            this_program.active_curriculum_uuid, that_program.active_curriculum_uuid
        )
    ...
    def test_get_discovery_program_success(self):
        ...
        """
        responses.add(
            responses.GET, self.discovery_url, json=self.programs_response, status=200
        )
...
class DiscoveryProgramTests(TestCase):
    ...
    def setUp(self):
        ...
        self.program = DiscoveryProgram(
            ...
            program_type=program_type,
            active_curriculum_uuid=curriculum_uuid,
            course_runs=[self.make_course_run(i) for i in range(4)],
        )
    ...
    def test_get_key(self, attr):
        for course_run in self.program.course_runs:
            ...
            self.assertEqual(self.program.get_course_key(test_key), course_run.key)
    ...
    def test_get_key_not_found(self):
        for i in [10, 101, 111, 123]:
            ...
            self.assertIsNone(
                self.program.get_course_key(not_in_program_run.external_key)
            )
    ...
    def test_get_external_key(self, attr):
        for course_run in self.program.course_runs:
            test_key = getattr(course_run, attr)
            self.assertEqual(
                self.program.get_external_course_key(test_key), course_run.external_key
            )
    ...
    def test_get_external_key_not_found(self):
        for i in [10, 101, 111, 123]:
            not_in_program_run = self.make_course_run(i)
            self.assertIsNone(
                self.program.get_external_course_key(not_in_program_run.key)
            )
