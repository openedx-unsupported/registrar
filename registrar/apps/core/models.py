...
class OrganizationGroup(Group):
    ...
    ROLE_CHOICES = ((role.name, role.description) for role in perms.ORGANIZATION_ROLES)
...
class ProgramOrganizationGroup(Group):
    ...
    ROLE_CHOICES = ((role.name, role.description) for role in perms.PROGRAM_ROLES)

    program = models.ForeignKey(Program, on_delete=models.CASCADE)
    granting_organization = models.ForeignKey(Organization, on_delete=models.CASCADE)
    role = models.CharField(
        max_length=255, choices=ROLE_CHOICES, default=perms.ProgramReadMetadataRole.name
    )
    ...
    def __str__(self):
        return "ProgramOrganizationGroup: {} role={}".format(
            self.program.title, self.role
        )  # pragma: no cover
...
class PendingUserGroup(TimeStampedModel):
    ...
    def __str__(self):
        ...
        return "<PendingUserGroup {ID}>: {group_name} - {user_email}".format(
            ID=self.id, group_name=self.group.name, user_email=self.user_email
        )
...
class JobPermissionSupport(models.Model):
    ...
    class Meta:
        ...
        permissions = ((perms.JOB_GLOBAL_READ_KEY, "Global Job status reading"),)
