...
class TestManagePrograms(TestCase):
    ...
    def test_create_program(self):
        ...
        call_command(
            self.command, self._uuidkeys((self.arabic_uuid, "masters-in-arabic"))
        )
    ...
    def test_create_program_no_key(self):
        self.mock_get_discovery_program.return_value = self.arabic_discovery_program
        self.assert_program_nonexistant(self.arabic_uuid)
        call_command(self.command, self.arabic_uuid)
    ...
    def test_create_programs_some_key(self):
        ...
        call_command(
            self.command,
            self._uuidkeys((self.arabic_uuid, "masters-in-arabic"))
            + ","
            + self.russian_uuid,
        )
        self.assert_program(self.arabic_uuid, "masters-in-arabic", self.org)
    ...
    def test_modify_program(self):
        ...
        call_command(
            self.command, self._uuidkeys((self.english_uuid, "english-program"))
        )
    ...
    def test_modify_program_do_nothing(self):
        ...
        self.assert_program(self.english_uuid, "masters-in-english", self.org)
        call_command(
            self.command, self._uuidkeys((self.english_uuid, "masters-in-english"))
        )
        self.assert_program(self.english_uuid, "masters-in-english", self.org)
    ...
    def test_no_authoring_orgs(self, authoring_organizations):
        ...
        with self.assertRaisesRegex(
            CommandError, "No authoring org keys found for program"
        ):
            call_command(
                self.command, self._uuidkeys((self.english_uuid, "english-program"))
            )
    ...
    def test_org_not_found(self):
        self.mock_get_discovery_program.return_value = self.discovery_dict(
            "nonexistant-org", self.english_uuid, "english-slug"
        )
        # pylint: disable=deprecated-method
        with self.assertRaisesRegex(
            CommandError, "None of the authoring organizations (.*?) were found"
        ):
            call_command(
                self.command, self._uuidkeys((self.english_uuid, "english_program"))
            )
