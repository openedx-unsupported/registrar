...
def list_program_enrollments(self, job_id, user_id, file_format, program_key):
    ...
    try:
        ...
        post_job_failure(job_id, "Invalid enrollment data from LMS: {}".format(err))
        return
...
def list_course_run_enrollments(
    ...
    try:
        enrollments = data.get_course_run_enrollments(
            program.discovery_uuid, internal_course_key, external_course_key
        )
    except HTTPError as err:
        post_job_failure(
            job_id,
...
def list_all_course_run_enrollments(self, job_id, user_id, file_format, program_key):
    ...
    for course_run in program.discovery_program.course_runs:
        try:
            enrollments = data.get_course_run_enrollments(
                program.discovery_uuid, course_run.key, course_run.external_key
            )
        except HTTPError as err:
            post_job_failure(
                job_id,
...
def write_program_enrollments(self, job_id, user_id, json_filepath, program_key):
    """
...
CourseEnrollmentResponseItem = namedtuple(
    "CourseEnrollmentResponseItem", ["course_id", "student_key", "status"]
)


@shared_task(base=EnrollmentWriteTask, bind=True)
def write_course_run_enrollments(self, job_id, user_id, json_filepath, program_key):
    """
    ...
    for requested_course_key, course_requests in requests_by_course_key.items():
        ...
        internal_course_key = program.discovery_program.get_course_key(
            ...
            course_responses.extend(
                [
                    CourseEnrollmentResponseItem(
                        requested_course_key, student_key, status
                    )._asdict()
    ...
    results_str = serialize_to_csv(
        course_responses, CourseEnrollmentResponseItem._fields, include_headers=True
    )
    post_job_success(job_id, results_str, "csv", text=code_str)
