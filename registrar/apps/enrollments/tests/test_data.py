...
class GetEnrollmentsTestMixin(object):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.good_input_1 = [
            {"student_key": "abcd", "account_exists": True, "status": cls.status(0)},
            {"student_key": "efgh", "account_exists": False, "status": cls.status(4)},
        ]
        cls.good_input_2 = [
            {"student_key": "ijkl", "account_exists": False, "status": cls.status(2)},
            {"student_key": "mnop", "account_exists": True, "status": cls.status(3)},
        ]
        cls.bad_input = [
            {
                "student_key": "qrst",
        ...
        cls.good_output = [
            enrollment.copy() for enrollment in cls.good_input_1 + cls.good_input_2
        ]
...
class GetProgramEnrollmentsTestCase(GetEnrollmentsTestMixin, TestCase):
    ...
    lms_url = urljoin(
        settings.LMS_BASE_URL, LMS_PROGRAM_ENROLLMENTS_API_TPL.format(program_uuid)
    )
...
class GetCourseRunEnrollmentsTestCase(GetEnrollmentsTestMixin, TestCase):
    ...
    lms_url = urljoin(
        ...
        LMS_PROGRAM_COURSE_ENROLLMENTS_API_TPL.format(program_uuid, course_id),
    )
    ...
    def test_external_course_key(self, external_course_key):
        ...
        enrollments = get_course_run_enrollments(
            self.program_uuid, self.course_id, external_course_key
        )
        for enrollment in enrollments:
            self.assertEqual(
                enrollment.get("course_id"), external_course_key or self.course_id
            )
...
class WriteEnrollmentsTestMixin(object):
    ...
    @mock_oauth_login
    @responses.activate
    def test_write_enrollments(self, lms_write_size, lms_statuses, expected_good):
        ...
        for call in responses.calls[1:]:
            ...
            if self.curriculum_uuid:
                for enrollment in body:
                    self.assertEqual(
                        enrollment.get("curriculum_uuid"), self.curriculum_uuid
                    )
    ...
    def test_backend_errors(self, mock_data_logger):
        ...
        dict_400 = {"developer_message": "this is a dict error from lms"}
...
class WriteProgramEnrollmentsTests(WriteEnrollmentsTestMixin, TestCase):
    ...
    def url(self):
        return urljoin(
            settings.LMS_BASE_URL,
            LMS_PROGRAM_ENROLLMENTS_API_TPL.format(self.program_uuid),
        )
    ...
    def url(self):
        return urljoin(
            ...
            LMS_PROGRAM_COURSE_ENROLLMENTS_API_TPL.format(
                self.program_uuid, self.course_key
            ),
        )
