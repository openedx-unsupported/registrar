...
class TrackTestMixin(object):
    ...
    * to be provided fields self.user and self.user_org
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.segment_patcher = mock.patch(
            "registrar.apps.api.segment.track", autospec=True
        )
        cls.logging_patcher = mock.patch(
            "registrar.apps.api.mixins.logger", autospec=True
        )
    ...
    def assert_tracking(self, user=None, event=None, status_code=None, **kwargs):
        ...
        self.mock_segment_track.assert_called_once_with(
            user.username, event, properties
        )
...
class JwtMixin(object):
    ...
    def generate_jwt_header(self, user, admin=False, ttl=5, **overrides):
        ...
        jwt_token = self.generate_id_token(user, admin, ttl, **overrides).decode(
            "utf-8"
        )
