...
from registrar.apps.enrollments.data import LMS_PROGRAM_COURSE_ENROLLMENTS_API_TPL
...
class RegistrarAPITestCase(TrackTestMixin, APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.cs_program = ProgramFactory(
            key="masters-in-cs", managing_organization=cls.stem_org
        )
        cls.mech_program = ProgramFactory(
            key="masters-in-me", managing_organization=cls.stem_org
        )
        ...
        cls.phil_program = ProgramFactory(
            key="masters-in-philosophy", managing_organization=cls.hum_org
        )
        cls.english_program = ProgramFactory(
            key="masters-in-english", managing_organization=cls.hum_org
        )
    ...
    def _add_program_to_cache(
        self, program, title=None, url=None, program_type=None, curricula=None
    ):
        """
        ...
        cache.set(
            ...
            self._discovery_program(
                program.discovery_uuid, title, url, program_type, curricula
            ),
        )
...
class ProgramListViewTests(RegistrarAPITestCase, AuthRequestMixin):
    ...
    @ddt.data(
        ...
        {"groups": set(), "perm_filter": "read_metadata"},
        {"groups": set(), "perm_filter": "read_enrollments"},
        {"groups": set(), "perm_filter": "write_enrollments"},
        {
            "groups": set(),
        ...
        {"groups": {"stem-users", "hum-ops"}, "perm_filter": "write_enrollments"},
        {
    ...
    def test_program_filters(
        ...
        org_or_program_groups = (
            [ProgramOrganizationGroup.objects.get(name=name) for name in groups]
            if test_program_group
            else [OrganizationGroup.objects.get(name=name) for name in groups]
        )
    ...
    def test_program_filter_with_mm_programs(
        self, groups, perm_filter, expected_programs, global_perm=False
    ):
...
class ProgramRetrieveViewTests(RegistrarAPITestCase, AuthRequestMixin):
    ...
    @ddt.data(
        ...
        (
            ...
            ["read_metadata", "read_reports", "read_enrollments", "write_enrollments"],
        ),
    )
    @ddt.unpack
...
class ProgramCourseListViewTests(RegistrarAPITestCase, AuthRequestMixin):
    ...
    def test_get_program_with_no_course_runs(self):
        ...
        disco_program = self._discovery_program(
            ...
            self.program_url,
            self.program_type,
            [{"is_active": True, "courses": [{"course_runs": []}]}],
        )
    ...
    def test_get_program_with_no_active_curriculum(self):
        ...
        disco_program = self._discovery_program(
            ...
            self.program_url,
            self.program_type,
            [{"is_active": False, "courses": [{"course_runs": []}]}],
        )
...
class ProgramEnrollmentWriteMixin(object):
    ...
    def setUpTestData(cls):  # pylint: disable=missing-docstring
        ...
        cls.disco_program = DiscoveryProgram.from_json(
            program_uuid,
            {
                "curricula": [
        ...
        cls.program_no_curricula = DiscoveryProgram.from_json(
            program_uuid, {"curricula": [], "type": "Masters"}
        )
    ...
    def student_enrollment(self, status, student_key=None):
        return {"status": status, "student_key": student_key or uuid.uuid4().hex[0:10]}

    def test_program_unauthorized_at_organization(self):
        req_data = [self.student_enrollment("enrolled")]
    ...
    def test_successful_program_enrollment_write(self):
        expected_lms_response = {"001": "enrolled", "002": "enrolled", "003": "pending"}
        ...
        with self.assert_tracking(user=self.stem_admin, program_key="masters-in-cs"):
            with mock.patch.object(
                DiscoveryProgram, "get", return_value=self.disco_program
            ):
                response = self.request(
                    self.method,
                    "programs/masters-in-cs/enrollments/",
    ...
    def test_backend_unprocessable_response(self):
        ...
        self.mock_enrollments_response(
            self.method, expected_lms_response, response_code=422
        )
    ...
    def test_backend_multi_status_response(self):
        ...
        self.mock_enrollments_response(
            self.method, expected_lms_response, response_code=207
        )
        ...
        with self.assert_tracking(
            user=self.stem_admin, program_key="masters-in-cs", status_code=207
        ):
            with mock.patch.object(
                DiscoveryProgram, "get", return_value=self.disco_program
            ):
    ...
    @responses.activate
    @ddt.data(
        (500, "Internal Server Error"), (404, "Not Found"), (413, "Payload Too Large")
    )
    @ddt.unpack
    def test_backend_request_failed(self, status_code, content):
        self.mock_enrollments_response(self.method, content, response_code=status_code)
        req_data = [
            self.student_enrollment("enrolled", "001"),
            self.student_enrollment("enrolled", "002"),
            self.student_enrollment("pending", "003"),
    ...
    def test_discovery_404(self):
        req_data = [self.student_enrollment("enrolled", "001")]
...
class ProgramEnrollmentPostTests(
    ProgramEnrollmentWriteMixin, RegistrarAPITestCase, AuthRequestMixin
):
    method = "POST"
    event = "registrar.v1.post_program_enrollment"


class ProgramEnrollmentPatchTests(
    ProgramEnrollmentWriteMixin, RegistrarAPITestCase, AuthRequestMixin
):
    method = "PATCH"
...
class ProgramEnrollmentGetTests(S3MockMixin, RegistrarAPITestCase, AuthRequestMixin):
    ...
    enrollments = [
        {"student_key": "abcd", "status": "enrolled", "account_exists": True},
        {"student_key": "efgh", "status": "pending", "account_exists": False},
    ]
    ...
    def test_program_not_found(self):
        with self.assert_tracking(
            ...
            response = self.get(
                "programs/masters-in-polysci/enrollments", self.hum_admin
            )
        self.assertEqual(response.status_code, 404)
...
@ddt.ddt
class ProgramCourseEnrollmentGetTests(
    S3MockMixin, RegistrarAPITestCase, AuthRequestMixin
):
    """ Tests for GET /api/v1/programs/{program_key}/enrollments endpoint """
    ...
    program_uuid = str(uuid.uuid4())
    disco_program = DiscoveryProgram.from_json(
        program_uuid,
        {
            "curricula": [
                {
    ...
    @ddt.data(
        ...
        (
            "masters-in-polysci",
            "course-v1:HUMx+English-550+Spring",
            "program_not_found",
        ),
    ...
    def test_not_found(self, program_key, course_id, expected_failure, _mock):
        ...
        with self.assert_tracking(
            ...
            response = self.get(path_fmt.format(program_key, course_id), self.hum_admin)
        self.assertEqual(response.status_code, 404)
...
class JobStatusRetrieveViewTests(S3MockMixin, RegistrarAPITestCase, AuthRequestMixin):
    ...
    def test_successful_job(self):
        ...
        with self.assert_tracking(
            user=self.stem_admin, job_id=job_id, job_state="Succeeded"
        ):
    ...
    def test_failed_job(self, mock_jobs_logger):
        ...
        with self.assert_tracking(
            user=self.stem_admin, job_id=job_id, job_state="Failed"
        ):
            job_respose = self.get("jobs/" + job_id, self.stem_admin)
        self.assertEqual(job_respose.status_code, 200)
    ...
    def test_job_global_read_permission(self):
        ...
        with self.assert_tracking(
            user=self.hum_admin, job_id=job_id, job_state="Succeeded"
        ):
            job_respose = self.get("jobs/" + job_id, self.hum_admin)
        self.assertEqual(job_respose.status_code, 200)
    ...
    def test_job_does_not_exist(self):
        ...
        with self.assert_tracking(
            user=self.stem_admin, job_id=nonexistant_job_id, failure="job_not_found"
        ):
...
def _succeeding_job(
    self, job_id, user_id, *args, **kwargs
...
@shared_task(base=UserTask, bind=True)
def _failing_job(
    self, job_id, user_id, fail_message, *args, **kwargs
):  # pylint: disable=unused-argument
...
class ProgramCourseEnrollmentWriteMixin(object):
    ...
    def setUpTestData(cls):  # pylint: disable=missing-docstring
        ...
        cls.path = "programs/masters-in-english/courses/{}/enrollments".format(
            cls.course_id
        )
        cls.lms_request_url = urljoin(
    ...
    def mock_course_enrollments_response(
        self, method, expected_response, response_code=200
    ):
        self.mock_api_response(
            self.lms_request_url,
            expected_response,
    ...
    def test_program_unauthorized_at_organization(self):
        req_data = [self.student_course_enrollment("active")]
    ...
    def test_successful_program_course_enrollment_write(self, use_external_course_key):
        course_id = (
            self.external_course_key if use_external_course_key else self.course_id
        )
        expected_lms_response = {"001": "active", "002": "active", "003": "inactive"}
        ...
        with self.assert_tracking(
            user=self.stem_admin, program_key=self.cs_program.key, course_id=course_id
        ):
            response = self.request(
                self.method,
        ...
        self.assertCountEqual(
            lms_request_body,
            [
                {"status": "active", "student_key": "001"},
                {"status": "active", "student_key": "002"},
                {"status": "inactive", "student_key": "003"},
            ],
        )
        self.assertEqual(response.status_code, 200)
        self.assertDictEqual(response.data, expected_lms_response)
    ...
    @ddt.unpack
    def test_backend_request_failed(
        self, status_code, content, use_external_course_key
    ):
        course_id = (
            self.external_course_key if use_external_course_key else self.course_id
        )
    ...
    @ddt.data(
        ("this is a string", "expected request body type: List"),
        ({"this is a": "dict"}, "expected request body type: List"),
        (
            ["this enrollment is a string"],
            "expected items in request to be of type Dict",
        ),
        ([None], "expected items in request to be of type Dict"),
        (
...
class ProgramCourseEnrollmentPostTests(
    ProgramCourseEnrollmentWriteMixin, RegistrarAPITestCase, AuthRequestMixin
):
    method = "POST"
    event = "registrar.v1.post_course_enrollment"


class ProgramCourseEnrollmentPatchTests(
    ProgramCourseEnrollmentWriteMixin, RegistrarAPITestCase, AuthRequestMixin
):
    method = "PATCH"
...
class JobStatusListView(S3MockMixin, RegistrarAPITestCase, AuthRequestMixin):
    ...
    def create_dummy_job_status(cls, n, user, state):
        ...
        task_status = UserTaskStatus.objects.create(
            state=state, user=user, task_id=cls.job_ids[n], total_steps=1
        )
        return {
            "state": state,
    ...
    def test_some_jobs_processing(self):
        self._test_list_job_statuses(
            self.edx_admin, [self.edx_admin_jobs[0], self.edx_admin_jobs[2]]
        )
...
class EnrollmentUploadMixin(object):
    ...
    def _upload_enrollments(
        self, enrollments, user=None
        ...
        with mock.patch.object(
            ...
            return self.request(self.method, self.path, user, file=upload_file)
    ...
    def test_enrollment_upload_success(self):
        ...
        with self.assert_tracking(
            user=self.stem_admin, program_key=self.cs_program.key, status_code=202
        ):
            upload_response = self._upload_enrollments(enrollments)
        ...
        retrieved = filestore.retrieve(
            "/{}/{}.json".format("uploads", upload_response.data["job_id"])
        )
    ...
    def test_enrollment_upload_conflict(self):
        ...
        with mock.patch(
            "registrar.apps.api.v1.views.is_enrollment_write_blocked", return_value=True
        ):
            upload_response = self._upload_enrollments(enrollments)
    ...
    def test_enrollment_upload_invalid_header(self):
        ...
        self.csv_headers = (
            "something" if field == "student_key" else field
    ...
    def test_enrollment_upload_missing_data(self):
        enrollments = [self.build_enrollment("enrolled", "001"), {"student_key": "002"}]
        upload_response = self._upload_enrollments(enrollments)
    ...
    def test_unmatched_row_data(self):
        upload_file = StringIO()
        writer = csv.writer(
            upload_file, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
        )
        ...
        with mock.patch.object(
            ...
            upload_response = self.request(
                self.method, self.path, self.stem_admin, file=upload_file
            )
        self.assertEqual(upload_response.status_code, 400)

    def test_enrollment_upload_too_large(self):
        enrollments = [self.build_enrollment("pending", str(n)) for n in range(1000)]
    ...
    def test_no_file(self):
        with mock.patch.object(
            ...
            upload_response = self.request(
                self.method, self.path, self.stem_admin, file=None
            )
        self.assertEqual(upload_response.status_code, 400)
...
class ProgramEnrollmentUploadTest(
    EnrollmentUploadMixin, S3MockMixin, RegistrarAPITestCase, AuthRequestMixin
):
...
class CourseEnrollmentUploadTest(
    EnrollmentUploadMixin, S3MockMixin, RegistrarAPITestCase, AuthRequestMixin
):
...
class CourseEnrollmentDownloadTest(S3MockMixin, RegistrarAPITestCase, AuthRequestMixin):
    ...
    @mock.patch.object(DiscoveryProgram, "get", return_value=disco_program)
    @mock.patch("registrar.apps.enrollments.tasks.data.get_course_run_enrollments")
    @ddt.data(
        (None, "json", enrollments_json),
        ("json", "json", enrollments_json),
        ("csv", "csv", enrollments_csv),
    ...
    def test_program_not_found(self):
        with self.assert_tracking(
            ...
            response = self.get(
                "programs/masters-in-polysci/course_enrollments", self.hum_admin
            )
        self.assertEqual(response.status_code, 404)
...
class CourseGradeViewTest(S3MockMixin, RegistrarAPITestCase, AuthRequestMixin):
    ...
    grades = [
        ...
        {"student_key": "learner-03", "error": "error loading grades"},
    ]
...
class ReportsListViewTest(S3MockMixin, RegistrarAPITestCase, AuthRequestMixin):
    ...
    def tearDown(cls):
        ...
        files = filestore.list(
            "{}/{}".format(
                cls.hum_org.key, cls.uuid_hexify(cls.english_program.discovery_uuid)
            )
        )
        ...
        for file in files[1]:
            file_path = "{}/{}/{}".format(
                cls.hum_org.key,
                cls.uuid_hexify(cls.english_program.discovery_uuid),
                file,
            )
    ...
    def test_ok(self):
        files = [
            {"name": "aggregate_report__2019-12-18", "created_date": "2019-12-18"},
            {"name": "individual_report__2019-12-17", "created_date": "2019-12-17"},
            {"name": "individual_report__2019-12-18.csv", "created_date": "2019-12-18"},
        ]

        file_prefix = "{}/{}".format(
            self.hum_org.key, self.uuid_hexify(self.english_program.discovery_uuid)
        )
        filestore = get_program_reports_filestore()
        for file in files:
            filestore.store("{}/{}".format(file_prefix, file["name"]), "data")
    ...
    def test_min_created_date_query_parameter(self):
        files = [
            {"name": "aggregate_report__2019-12-17", "created_date": "2019-12-17"},
            {"name": "aggregate_report__2019-12-18", "created_date": "2019-12-18"},
            {"name": "individual_report__2019-12-17", "created_date": "2019-12-17"},
            {"name": "individual_report__2019-12-18", "created_date": "2019-12-18"},
        ]
        ...
        expected_data = [
            {
                ...
                "download_url": filestore.get_url(
                    "{}/{}".format(file_prefix, "aggregate_report__2019-12-18")
                ),
            },
            {
                ...
                "created_date": "2019-12-18",
                "download_url": filestore.get_url(
                    "{}/{}".format(file_prefix, "individual_report__2019-12-18")
                ),
            },
        ]
    ...
    def test_filename_misformatted(self):
        ...
        filestore = get_program_reports_filestore()
        for file in files:
            filestore.store("{}/{}".format(file_prefix, file), "data")
    ...
    def test_invalid_date_in_filename(self):
        files = ["aggregate_report__2019-25-25", "individual_report__2019-25-25"]
        file_prefix = "{}/{}".format(
            self.hum_org.key, self.uuid_hexify(self.english_program.discovery_uuid)
        )
        filestore = get_program_reports_filestore()
