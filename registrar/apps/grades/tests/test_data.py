...
class GetCourseGradesTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.bad_input = [{"letter_grade": True, "passed": "probably not"}]
        cls.good_output = [
            grade.copy() for grade in cls.good_input_1 + cls.good_input_2
        ]
    ...
    @responses.activate
    def test_get_grades(
        self, status_1, status_2, expected_successes, expected_failures
    ):
        responses.add(
            responses.GET,
            self.lms_url,
    ...
    def test_no_data(self):
        responses.add(
            responses.GET, self.lms_url, status=204, json={"next": None, "results": []}
        )
        any_successes, any_failures, grades = get_course_run_grades(
            self.program_uuid, self.course_id
        )
    ...
    def test_json_decode(self):
        responses.add(responses.GET, self.lms_url, status=422, body="this is 422")
        with self.assertRaises(ValidationError):
            get_course_run_grades(self.program_uuid, self.course_id)
